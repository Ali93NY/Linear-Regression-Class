...
# Conclusion: two outliers, 3rd and 7th observation ( 3rd is on border)
...
# Remove the 7th obs and refit the model:
 ...
 influence.measures(lm(Y ~ X1+X2, data = data[-7,]))
# VIF
library(car) 
vif(reg) 
...
cor(X1[-7],X2[-7]) # the cor is much stronger
...
pairs(data1)
cor(data1)
# strong asscoation between Y and X2, X4

#2.
reg <- lm(Y ~ X1 + X2 + X3 + X4)
vif(reg)

# none of them above 5

#3. 
rstudent(reg)
qt(.95, 81 -5-5) # 1-(alpha/2) very important

abs(rstudent(reg)) > qt(.95, 81 -5-5)
data2 <- data1[abs(rstudent(reg)) < qt(.95, 81 -5-5),]
nrow(data2)

# 10 obeservations are removed.

# 4
2*5/71
X = cbind(rep(1, nrow(data2)), X1, X2, X3, X4)
H = X%*%solve(t(X)%*%X)%*%t(X)
H
reg1 <- lm( Y ~ ., data = data2)
summary(reg1)
influence.measures(reg1)

im = influence.measures(reg1)
im$infmat[,9] > 0.1408451
which(im$infmat[,9] > 0.1408451)
...
# those varaibles are not prefect and needs modification
